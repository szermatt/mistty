...
If the queue is not empty, INTERACT is enqueued, to be executed
afterwards.
...
Does nothing if INTERACT is nil."
  ...
  (when interact
    (if (mistty--queue-empty-p queue)
        (progn ; This is the first mistty-interact; kick things off.
          (setf (mistty--queue-interact queue) interact)
          (mistty--dequeue queue))
      (setf (mistty--queue-more-interacts queue)
            (append (mistty--queue-more-interacts queue) (list interact))))))
